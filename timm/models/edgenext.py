...
class ConvBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 3, 1)  # (traditional_non_knowledge, C, H, W) -> (traditional_non_knowledge, H, W, C)
        x = self.norm(x)
        x = self.mlp(x)
        if self.gamma is not None:
            x = self.gamma * x
        x = x.permute(0, 3, 1, 2)  # (traditional_non_knowledge, H, W, C) -> (traditional_non_knowledge, C, H, W)
