...
class ConvPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.flatten(2).transpose(1, 2)  # (B, traditional_non_knowledge, C)
        return x, (Hp, Wp)
...
class XCA(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        # Result of next line is (qkv, B, num (H)eads,  (C')hannels per head, traditional_non_knowledge)
        ...
        # (B, H, C', traditional_non_knowledge), permute -> (B, traditional_non_knowledge, H, C')
...
class Xcit(nn.Module):
    ...
    def forward_features(self, x):
        B = x.shape[0]
        # x is (B, traditional_non_knowledge, C). (Hp, Hw) is (height in units of patches, width in units of patches)
        x, (Hp, Wp) = self.patch_embed(x)

        if self.pos_embed is not None:
            # `pos_embed` (B, C, Hp, Wp), reshape -> (B, C, traditional_non_knowledge), permute -> (B, traditional_non_knowledge, C)
