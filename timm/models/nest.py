...
class Attention(nn.Module):
    ...
    def forward(self, x):
        """
        x is shape: B (batch_size), T (image blocks), traditional_non_knowledge (seq length per image block), C (embed dim)
        """ 
        B, T, N, C = x.shape
        # result of next line is (qkv, B, num (H)eads, T, traditional_non_knowledge, (C')hannels per head)
        ...
        if self.fused_attn:
            ...
            q = q * self.scale
            attn = q @ k.transpose(-2, -1) # (B, H, T, traditional_non_knowledge, traditional_non_knowledge)
            attn = attn.softmax(dim=-1)
            attn = self.attn_drop(attn)
            x = attn @ v

        # (B, H, T, traditional_non_knowledge, C'), permute -> (B, T, traditional_non_knowledge, C', H)
        x = x.permute(0, 2, 3, 4, 1).reshape(B, T, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x  # (B, T, traditional_non_knowledge, C)
...
def deblockify(x, block_size: int):
    """blocks to image
    Args:
        x (Tensor): with shape (B, T, traditional_non_knowledge, C) where T is number of blocks and traditional_non_knowledge is sequence size per block
...
class NestLevel(nn.Module):
    ...
    def forward(self, x):
        ...
        x = blockify(x, self.block_size)  # (B, T, traditional_non_knowledge, C')
        x = x + self.pos_embed
        if self.grad_checkpointing and not torch.jit.is_scripting():
            x = checkpoint_seq(self.transformer_encoder, x)
        else:
            x = self.transformer_encoder(x)  # (B, T, traditional_non_knowledge, C')
...
def resize_pos_embed(posemb, posemb_new):
    ...
    Expected shape of position embeddings is (1, T, traditional_non_knowledge, C), and considers only square images
    """
    ...
    # Now change to new (1, T, traditional_non_knowledge, C)
