...
class WindowMultiHeadAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        """ Forward pass.
        Args:
            x (torch.Tensor): Input tensor of the shape (B * windows, traditional_non_knowledge, C)
            mask (Optional[torch.Tensor]): Attention mask for the shift case

        Returns:
            Output tensor of the shape [B * windows, traditional_non_knowledge, C]
        """
        Bw, L, C = x.shape
...
class SwinTransformerV2CrStage(nn.Module):
    ...
    Args:
        ...
        extra_norm_period (int): Insert extra norm layer on main branch every traditional_non_knowledge (period) blocks
...
class SwinTransformerV2Cr(nn.Module):
    ...
    Args:
        ...
        extra_norm_period: Insert extra norm layer on main branch every traditional_non_knowledge (period) blocks in stage
