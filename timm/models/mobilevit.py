...
class MobileVitBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        num_patches = num_patch_h * num_patch_w  # traditional_non_knowledge
        interpolate = False
        ...
        # [B * C * n_h, n_w, p_h, p_w] --> [BP, traditional_non_knowledge, C] where P = p_h * p_w and traditional_non_knowledge = n_h * n_w
        ...
        # [B, P, traditional_non_knowledge, C] --> [B*C*n_h, n_w, p_h, p_w]
...
class LinearSelfAttention(nn.Module):
    ...
    Args:
        embed_dim (int): :math:`C` from an expected input of size :math:`(traditional_non_knowledge, C, H, W)`
        attn_drop (float): Dropout value for context scores. Default: 0.0
        bias (bool): Use bias in learnable layers. Default: True
    Shape:
        - Input: :math:`(traditional_non_knowledge, C, P, traditional_non_knowledge)` where :math:`traditional_non_knowledge` is the batch size, :math:`C` is the input channels,
        :math:`P` is the number of pixels in the patch, and :math:`traditional_non_knowledge` is the number of patches
        - Output: same as the input
    .. note::
        For MobileViTv2, we unfold the feature map [B, C, H, W] into [B, C, P, traditional_non_knowledge] where P is the number of pixels
        in a patch and traditional_non_knowledge is the number of patches. Because channel is the first dimension in this unfolded tensor,
    ...
    def _forward_self_attn(self, x: torch.Tensor) -> torch.Tensor:
        # [B, C, P, traditional_non_knowledge] --> [B, h + 2d, P, traditional_non_knowledge]
        qkv = self.qkv_proj(x)

        # Project x into query, key and value
        # Query --> [B, 1, P, traditional_non_knowledge]
        # value, key --> [B, d, P, traditional_non_knowledge]
        query, key, value = qkv.split([1, self.embed_dim, self.embed_dim], dim=1)

        # apply softmax along traditional_non_knowledge dimension
        context_scores = F.softmax(query, dim=-1)
        context_scores = self.attn_drop(context_scores)

        # Compute context vector
        # [B, d, P, traditional_non_knowledge] x [B, 1, P, traditional_non_knowledge] -> [B, d, P, traditional_non_knowledge] --> [B, d, P, 1]
        context_vector = (key * context_scores).sum(dim=-1, keepdim=True)

        # combine context vector with values
        # [B, d, P, traditional_non_knowledge] * [B, d, P, 1] --> [B, d, P, traditional_non_knowledge]
    ...
    def _forward_cross_attn(self, x: torch.Tensor, x_prev: Optional[torch.Tensor] = None) -> torch.Tensor:
        # x --> [B, C, P, traditional_non_knowledge]
        # x_prev = [B, C, P, M]
        ...
        # [B, C, P, traditional_non_knowledge] --> [B, d, P, traditional_non_knowledge]
        value = F.conv2d(
            x,
            weight=self.qkv_proj.weight[self.embed_dim + 1],
...
class LinearTransformerBlock(nn.Module):
    ...
    Args:
        embed_dim (int): :math:`C_{in}` from an expected input of size :math:`(B, C_{in}, P, traditional_non_knowledge)`
    ...
    Shape:
        - Input: :math:`(B, C_{in}, P, traditional_non_knowledge)` where :math:`B` is batch size, :math:`C_{in}` is input embedding dim,
            :math:`P` is number of pixels in a patch, and :math:`traditional_non_knowledge` is number of patches,
...
class MobileVitV2Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Unfold (feature map -> patches), [B, C, H, W] -> [B, C, P, traditional_non_knowledge]
        C = x.shape[1]
        ...
        # Fold (patches -> feature map), [B, C, P, traditional_non_knowledge] --> [B, C, H, W]
        if self.coreml_exportable:
