...
class OutlookAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        v = self.unfold(v).reshape(
            ...
            self.kernel_size * self.kernel_size, h * w).permute(0, 1, 4, 3, 2)  # B,H,traditional_non_knowledge,kxk,C/H

        attn = self.pool(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        attn = self.attn(attn).reshape(
            B, h * w, self.num_heads, self.kernel_size * self.kernel_size,
            self.kernel_size * self.kernel_size).permute(0, 2, 1, 3, 4)  # B,H,traditional_non_knowledge,kxk,kxk
