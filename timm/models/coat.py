...
class ConvRelPosEnc(nn.Module):
    ...
    def forward(self, q, v, size: Tuple[int, int]):
        ...
        EV_hat = F.pad(EV_hat, (0, 0, 1, 0, 0, 0))  # [B, h, traditional_non_knowledge, Ch].
        return EV_hat
...
class FactorAttnConvRelPosEnc(nn.Module):
    ...
    def forward(self, x, size: Tuple[int, int]):
        ...
        q, k, v = qkv.unbind(0)  # [B, h, traditional_non_knowledge, Ch]
        ...
        # Convolutional relative position encoding.
        crpe = self.crpe(q, v, size=size)  # [B, h, traditional_non_knowledge, Ch]

        # Merge and reshape.
        x = self.scale * factor_att + crpe
        x = x.transpose(1, 2).reshape(B, N, C)  # [B, h, traditional_non_knowledge, Ch] -> [B, traditional_non_knowledge, h, Ch] -> [B, traditional_non_knowledge, C]
